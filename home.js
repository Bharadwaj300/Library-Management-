...
const showBooks = (books, prompt) => {
    ...
    document.getElementById("error").innerHTML = prompt +" " +( value>27 ? 27:value)+ " results";
}
...
window.onscroll = function (ev) {
    if ((window.innerHeight + window.scrollY) >= document.body.offsetHeight) {
        ...
        if (value <= 27) {
            const myTimeout = setTimeout(loading, 50);
            ...
            function loading() {
                ...
                setTimeout(() => {
                    ...
                    value = value + 3;
                    var filerbooks = []
                    for (var i = 0; i < value && i < 27; ++i) {

                        filerbooks.push(books[i]);

                    }
                    showBooks(filerbooks, "Showing");
